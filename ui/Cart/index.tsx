...
const CheckoutComponent = ({ cart }: CheckoutComponentProps) => {
  ...
  const style = 'flex flex-col items-center gap-2 p-4 rounded  w-full cursor-pointer border-2 transition hover:border-blue-600';
  const handleOnCheckout = () => {
    if(cart.cartItems.length > 0){
      router.push(`/checkout/${isNewCustomer ? 'newCustomer' : 'login'}`)
    }
  };
  const router = useRouter()
  return (
    <div className="flex flex-col gap-8 mt-4 w-2/5">
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="flex flex-col gap-4 pt-4 w-full">
          <div>Välj om du är en ny eller existerande kund:</div>
          <div className="flex gap-4 w-full">
            ...
            >
              <UserPlus size={32} className={isNewCustomer ? 'text-blue-600': 'text-gray-800'}/>
            ...
            >
              <LockKey size={32} className={!isNewCustomer ? 'text-blue-600': 'text-gray-800'} />
...
}> = ({ data, onRemoveItem, onUpdateQuantity }) => {
  const dispatch = useNextshopApiDispatch();

  return (
    <>
      {" "}
      <div className="max-w-7xl mx-auto p-4 flex gap-8 w-full">
